...
def test_build_datasets_no_yaml_load():
    ...
    if "yaml.safe_load" in content:
        import pytest
        pytest.fail("build_datasets.py contains yaml.safe_load - config should be passed as parameter")
    
    # Check for open(...config.yaml...)
    if re.search(r'open\([^)]*config\.yaml', content):
        import pytest
        pytest.fail("build_datasets.py opens config.yaml directly - config should be passed as parameter")
