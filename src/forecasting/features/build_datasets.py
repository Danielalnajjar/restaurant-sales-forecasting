...
def build_train_datasets(
    ...
    if len(df_train_short) > 0:
        ...
        logger.info(f"Saved short-horizon training data to {output_short_path} ({len(df_train_short)} rows, {len(df_train_short.columns)} columns)")
    ...
    if len(df_train_long) > 0:
        ...
        logger.info(f"Saved long-horizon training data to {output_long_path} ({len(df_train_long)} rows, {len(df_train_long.columns)} columns)")
...
def build_inference_features_2026(
    ...
    output_short_path: str = "data/processed/inference_features_short_2026.parquet",
    output_long_path: str = "data/processed/inference_features_long_2026.parquet",
) -> tuple:
    """
    Build inference features for 2026 forecasting.
    ...
    """
    logger.info("Building 2026 inference features")
    ...
    logger.info(f"Saved short-horizon 2026 features to {output_short_path} ({len(df_inf_short)} rows)")
...
if __name__ == "__main__":
    ...
    print(f"Short: {len(df_train_short)} rows, horizons {df_train_short['horizon'].min()}-{df_train_short['horizon'].max()}")
    print(f"Long: {len(df_train_long)} rows, horizons {df_train_long['horizon'].min()}-{df_train_long['horizon'].max()}")
