...
def compute_weekday_baseline(df_sales: pd.DataFrame, target_ds: pd.Timestamp, lookback_weeks: int = 8) -> float:
    """
...
def compute_event_uplift_priors(
    ...
    for _, event in df_events.iterrows():
        ...
        prior_mean = 1.0
        uplift_mean_shrunk = (n_days / (n_days + shrink_k)) * uplift_mean_raw + (shrink_k / (n_days + shrink_k)) * prior_mean
    ...
    logger.info(f"  Missing/no baseline: {df_uplift['confidence_bucket'].isin(['missing', 'no_baseline']).sum()}")
...
"""
    ...
    top_positive = df_uplift[df_uplift['uplift_mean_shrunk'].notna()].nlargest(10, 'uplift_mean_shrunk')
    ...
    top_negative = df_uplift[df_uplift['uplift_mean_shrunk'].notna()].nsmallest(10, 'uplift_mean_shrunk')
